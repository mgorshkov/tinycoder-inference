...
int main(int argc, char **argv) {
    ...
    const bool usePromptFile = strcmp(argv[2], "--file") == 0;
    const std::string prompt = (useTokensFile || usePromptFile) ? "" : argv[2];
    const char *tokensFilePath = useTokensFile && argc >= 4 ? argv[3] : nullptr;
    const char *promptFilePath = usePromptFile && argc >= 4 ? argv[3] : nullptr;
    std::string promptBuf;
    if (usePromptFile) {
        FILE *pf = fopen(promptFilePath, "rb");
        if (!pf) { fprintf(stderr, "cannot open prompt file %s\n", promptFilePath); return 1; }
        fseek(pf, 0, SEEK_END);
        long sz = ftell(pf);
        fseek(pf, 0, SEEK_SET);
        promptBuf.resize((size_t) sz);
        if (sz > 0 && fread(&promptBuf[0], 1, (size_t) sz, pf) != (size_t) sz) {
            fprintf(stderr, "prompt file read error\n");
            fclose(pf);
            return 1;
        }
        fclose(pf);
        if (argc >= 3) { /* --file <path> consumed above */ }
    }
    const char *promptCStr = usePromptFile ? promptBuf.c_str() : prompt.c_str();
    const int promptLen = usePromptFile ? (int) promptBuf.size()
                                        : (int) prompt.size();
    ...
    if (useTokensFile) {
        ...
        for (int pass = 0; pass < 2; ++pass) {
            ...
            int n = llama_tokenize(vocab, promptCStr, promptLen,
        ...
        nTok = llama_tokenize(vocab, promptCStr, promptLen,
    ...
    // not, the recurrence implementation differs between chunked/AR.
    {
        ...
        if (ctx2) {
            ...
            // AR final logits (last token) — apples-to-apples with TinyCoder's
            // per-token forward() top-k.
            {
                float *arl = llama_get_logits_ith(ctx2, 0);
                if (arl) {
                    std::vector<std::pair<float, int32_t>> scored;
                    scored.reserve(n_vocab);
                    for (int64_t i = 0; i < n_vocab; ++i) scored.emplace_back(arl[i], (int32_t) i);
                    std::partial_sort(scored.begin(), scored.begin() + 10, scored.end(),
                                      [](const auto &a, const auto &b) { return a.first > b.first; });
                    printf("  [ar] final logits (last prompt token) top-10:\n");
                    for (int i = 0; i < 10; ++i) {
                        const auto &p = scored[i];
                        std::string txt = pieceToString(vocab, p.second, false);
                        printf("    top%d: id=%d logit=%12.6f text=\"%s\"\n", i + 1,
                               p.second, p.first, txt.c_str());
                    }
                } else {
                    printf("  [ar] logits unavailable\n");
                }
            }
